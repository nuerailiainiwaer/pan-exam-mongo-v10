...
getJSON('/api/attempts/'+attemptId+'/result').then(function(r){
  meta.textContent = 'Score: '+r.scorePct+'% ('+r.correctCount+'/'+r.total+')  •  Time used: '+fmtDur(r.timeUsedSec)+'  •  Pass: '+(r.pass?'YES':'NO');
  grid.innerHTML='';
...
function showDetail(i, it){
  ...
  for(var k=0;k<opts.length;k++){
    ...
    if(it.answer && key===it.answer && it.answer!==it.correct){ txt+='  (your choice ✗)'; }
    if(it.answer && key===it.answer && it.answer===it.correct){ txt+='  (your choice ✓)'; }
